...
const executeTest = (module) => {
    let hashTmpFolder = hashString(localDir) + "/"
    // console.log(hashTmpFolder);
    tmpDir = path.join(tmpDir, hashTmpFolder);
...
const executeGenerate= (module) => {
    ...
    // TODO use getTranslationJson to check if there could be an issue with overwriting
...
const translateTest = (module) => {
    ...
    let moduleName = module.slice(0,-3);
...
const translateModule = (module) => {
    ...
    fs.outputFileSync(path.join(tmpDir, translatedModule), translatedContents);
...
const renderLess = (lessPath) => {
    ...
    return new Promise((resolve, reject) => {
       less.render(lessContentsCopy, (err, cssOutput) => {
            if (err == null) {
                fs.outputFileSync(path.join(tmpDir, "public/test.css"), cssOutput.css);
                resolve(true);
            } else {
                console.log(err);
                reject(false);
